...
import org.kopi.plotly.data.types.SplineData;
import org.kopi.plotly.layout.LayoutWithAxis;

import java.util.Arrays;

/**
 * Factory for spline charts
 *
 * @author rolea
 */
public class SplineChartFactory {

    private static final ILogger LOG = LogManager.getLogger(SplineChartFactory.class);

    /**
     * Builds a spline chart with the given title and data
     */
    public static final Component getSplineChart(String chartName, String xAxisLabel, String yAxisLabel, PlottableMap... lines) {
        DataConfiguration dataLine = new DataConfiguration();

        Arrays.stream(lines).forEach(line -> {
            LineData lineData = new SplineData(line.getLineName());
